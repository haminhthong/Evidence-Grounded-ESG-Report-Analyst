...
def _contains_any(text: str, keywords: tuple[str, ...]) -> bool:
    """Kiểm tra xem đoạn văn bản có chứa ít nhất một từ khóa hay không."""
...
def build_retrieval_plan(
    ...
    """Chuyển câu hỏi thành một kế hoạch truy xuất có kiểu dữ liệu rõ ràng."""
    lowered = question.lower()
    criteria = _infer_criteria(lowered)
    metrics = _infer_metrics(lowered)
    reporting_years = sorted({int(year) for year in re.findall(r"\b20[12]\d\b", question)})

    if any(word in lowered for word in ("compare", "versus", " vs ", "vs", "so sánh", "đối chiếu")):
        intent = "cross_document_compare"
        subqueries = [f"{question} {metric.replace('_', ' ')}" for metric in metrics] or [question]
        ...
        word in lowered
        for word in ("audit", "kiểm toán", "đánh giá toàn diện", "coverage", "bao phủ")
    ):
...
def _infer_metrics(question: str) -> list[str]:
    ...
    return [
        metric for metric, keywords in metric_keywords.items() if _contains_any(question, keywords)
    ]
